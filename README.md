...
- An SSH key uploaded to Hetzner (for `server create --ssh-key` and `server provision`)
...
Run this once to create a snapshot with Docker, gVisor, and the idle-shutdown daemon pre-installed. Every future `server create` boots from it in ~10s.

```bash
# 1. Boot a fresh server
sandbox server create --name setup-box --image ubuntu-24.04 --ssh-key <key-name> --wait

# 2. SSH in and install Docker + gVisor (see notes.md for the full script)
ssh root@<ip>

# 3. Install the idle-shutdown daemon (builds locally, copies via SCP, enables as systemd service)
sandbox server provision setup-box --identity ~/.ssh/id_ed25519

# 4. Snapshot and set as default boot image
sandbox snapshot create setup-box --power-off --set-default

# 5. Clean up the setup server
...
sandbox server provision <name> --identity <key-path> [--user root] [--arch amd64]
```
...
`sandbox server provision` installs it. The systemd unit is embedded in the binary at build time so no extra files are needed.
